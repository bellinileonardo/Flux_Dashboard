...
#st.write(hm.st.session_state)
mdls.menu_top_page()
st.subheader("📊 Flux Dashboard - Seu Varejo Integrado com IA")
st.caption("Esta analise de DEMONSTRAÇÂO usa a interface GEMINI do Google para analisar as vendas da loja. Os dados enviados são limitados a 800 registros, para evitar carga excessiva na IA")
...
if gemini_client:
    ...
    data_para_ia = data_para_ia.head(800)
    ...
    if st.button(f"Analisar com IA, {total_registros_enviados_ia} Registros\n - Data Inicial: {data_min_dado_enviado}\n - Data Final: {data_max_dado_enviado}", key="ia_button_gemini_flux", type="primary", use_container_width=True):
